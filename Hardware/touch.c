...
//返回值:读到的数据
#define READ_TIMES 12 	//读取次数（按钮点按无需高精度，降低单次扫描阻塞：原40次约8ms→12次约2.5ms）
...
//        0，获取失败，要重新校准
//
// 注意：本工程"EEPROM"是 STM32 内部 Flash 末页（0x0807F800）模拟而来。
//      该页 STM32 烧写新固件时不会被擦除，因此芯片若曾烧写过其他工程
//      （或本工程的旧版），可能残留 0x0A 标志位 + 旧校准参数。
//      为了避免使用过期/损坏的参数，这里在读出后再做合理性检查：
//        - xfac/yfac 必须在 (0.03, 1.0) 之间
//        - touchtype 必须是 0 或 1
//      任意一项不合理都视为未校准，强制进入十字校准流程。
u8 TP_Get_Adjdata(void)
{
	...
	if(tempfac==0X0A)//触摸屏已经校准过了
	{
		...
		/* === 校准值合理性检查（防止 Flash 残留的脏数据让人跳过校准） === */
		{
			float ax = tp_dev.xfac < 0 ? -tp_dev.xfac : tp_dev.xfac;
			float ay = tp_dev.yfac < 0 ? -tp_dev.yfac : tp_dev.yfac;
			if (ax < 0.03f || ax > 1.0f ||
			    ay < 0.03f || ay > 1.0f ||
			    tp_dev.touchtype > 1)
			{
				return 0;	/* 数据不合理 → 强制重新校准 */
			}
		}
...
{	
	...
	RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOB|RCC_APB2Periph_GPIOC|RCC_APB2Periph_GPIOF, ENABLE);	 //使能PB,PC,PF端口时钟

	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_0;				 // PC0端口配置 (TCLK)
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP; 		 //推挽输出
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
	GPIO_Init(GPIOC, &GPIO_InitStructure);//C0推挽输出
